...
class SimpleNetworkNodeTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.manager = User.objects.create_user(
            username="manager", password="managerpass123", is_active=True
        )
        # Если у тебя есть кастомная логика прав, добавь ее здесь

        # Создаем продукты с обязательным release_date
        self.product1 = Product.objects.create(
            name="Телефон", model="X100", release_date=timezone.now().date()
        )
        self.product2 = Product.objects.create(
            name="Ноутбук", model="Y200", release_date=timezone.now().date()
        )
    ...
    def test_get_supplier_chain(self):
        ...
        url = reverse(
            "network:network-node-supplier-chain", kwargs={"pk": self.store.pk}
        )
        response = self.client.get(url)
    ...
    def test_get_available_supplier_products(self):
        ...
        url = reverse(
            "network:network-node-available-supplier-products",
...
class SimpleProductTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.product1 = Product.objects.create(
            name="Телевизор", model="SmartTV 55", release_date=timezone.now().date()
        )
        self.product2 = Product.objects.create(
            name="Холодильник", model="Cold 200", release_date=timezone.now().date()
        )
    ...
    def test_assign_product_to_node(self):
        ...
        data = {"node_id": self.node.id, "price": 1000.00, "quantity": 5}
        response = self.client.post(url, data)
...
class BasicModelTests(APITestCase):
    ...
    def test_create_node_product_relation(self):
        ...
        product = Product.objects.create(
            name="Продукт", model="Model", release_date=timezone.now().date()
        )
